...
function M.resolve(buf)
  ...
  local have_formatter = false

  ---@param formatter LazyFormatter
  return vim.tbl_map(function(formatter)
    local sources = formatter.sources(buf)
    local active = #sources > 0 and not have_formatter
    have_formatter = have_formatter or active or false
    return setmetatable({
      active = active,
      resolved = sources,
    }, { __index = formatter })
...
function M.toggle(buf)
  ...
  if buf then
    vim.b[bufnr].autoformat = not M.enabled()
  else
